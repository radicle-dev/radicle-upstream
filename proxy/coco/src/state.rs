...
impl State {
    ...
    pub async fn init_project<P: AsRef<path::Path> + Send + Sync + 'static>(
        ...
        // TODO(finto): We might need to wait on transport results here
        Ok(meta)
    }
        ...
        P: Into<Option<PeerId>> + Send + 'static,
    {
        ...
        let checkout = project::Checkout::new(proj, default_branch, destination, include_path);
...
#[allow(clippy::panic)]
mod test {
    use std::{convert::TryFrom, env, path::PathBuf, process::Command};
    ...
    fn fakie_project(path: PathBuf) -> project::Create<PathBuf> {
        project::Create {
            ...
            default_branch: OneLevel::from(RefLike::try_from("dope").unwrap()),
        }
    }
