...
mod handler {
  ...
  pub async fn create(
    ...
    let urn = meta.urn();
    let stats = peer
      .with_api(|api| coco::Peer::with_browser(api, &urn, |browser| Ok(browser.get_stats()?)))?;
  ...
  pub async fn register<R: registry::Client>(
    ...
    let project_name = registry::ProjectName::try_from(input.project_name).map_err(Error::from)?;
...
impl ToDocumentedType for project::Project {
  fn document() -> document::DocumentedType {
    ...
    properties.insert(
      "stats".into(),
      document::string().description("Branch, commit, and contributor counts on default branch"),
    );
...
impl Serialize for project::Registration {
    ...
    S: Serializer,
  {
    match self {
      ...
      Self::User(user_id) => {
        serializer.serialize_newtype_variant("Registration", 1, "User", &user_id.to_string())
      },
    }
  }
}
