...
mod handler {
    ...
    pub async fn get_project<R: registry::Client>(
        ...
        let reg = registry.read().await;
        let org_id = registry::Id::try_from(org_id).map_err(Error::from)?;
        let project_registrant = registry::ProjectRegistrant::Org(org_id);
        let project_name = registry::ProjectName::try_from(project_name).map_err(Error::from)?;
        let project = reg.get_project(project_registrant, project_name).await?;
        ...
        R: registry::Client,
    {
        ...
        for p in &projects {
            ...
            let org_project = super::Project {
                ...
                org_id: p.registrant.id().to_string(),
                shareable_entity_identifier: format!(
                    "%{}/{}",
                    p.registrant.id().to_string(),
                    p.name.to_string()
                ),
                maybe_project,
            };
...
impl Serialize for registry::Project {
        ...
        S: Serializer,
    {
        ...
        state.serialize_field("orgId", &self.registrantid().to_string())?;
...
mod test {
    ...
    async fn get_project() -> Result<(), error::Error> {
        ...
        let project_registrant = registry::ProjectRegistrant::Org(org_id.clone());
